...
let RenderRow = (props) => {
  ...
  const renderCell = (dataArray, parentCell) =>
      ...
      if (index < 1) {
        if (parentCell) {
          return (
            <TableCell
              key={index.toString()}
              style={{ paddingLeft: keyID.split("_").length * 20 }}
            >
              {arrowMore.indexOf(keyID) > -1
                ? renderIconMore(collapseIcon)
                : renderIconLess(expandIcon)}
              {keyID}
            </TableCell>
          );
        }
  ...
  const row = parent ? (
    <TableRow
      key={keyID}
      className={
        treeOpen.indexOf(keyID) < 0 && keyID.indexOf("_") > -1
          ? classes.hideRow
          : classes.anchor
      }
...
function TreeTable(props) {
  ...
  return (
    <Table className={classes.table}>
      <TableHead>
        <TableRow>{getHead(dataTable.head)}</TableRow>
      </TableHead>
      <TableBody>{getData(dataTable.body)}</TableBody>
    </Table>
  );
}
